...
public class DrivetrainSubsystem extends SubsystemBase {
  ...
  public void periodic() {
    ...
    SmartDashboard.putNumber("Front Right Steer Absolute Angle",
        Units.radiansToDegrees(m_frontRightModule.getSteerEncoder().getAbsoluteAngle()));
    SmartDashboard.putNumber("Back Left Steer Absolute Angle",
    ...
    if (SmartDashboard.getBoolean("Set drive PID", false)) {
      ...
      double steerP = SmartDashboard.getNumber("SwerveDrive Steer P", getSteerPID().getP());
      double steerI = SmartDashboard.getNumber("SwerveDrive Steer I", getSteerPID().getI());
      double steerD = SmartDashboard.getNumber("SwerveDrive Steer D", getSteerPID().getD());
      setDrivePID(p, i, d);
      setFrontRightSteerPID(steerP, steerI, steerD);
      SmartDashboard.putBoolean("Set drive PID", false);
    }
  }
  ...
  public void swerveState(boolean halfSpeed) {
    ...
    m_frontLeftModule.set(states[0].speedMetersPerSecond * scale / MAX_VELOCITY_METERS_PER_SECOND * MAX_VOLTAGE,
        states[0].angle.getRadians() - _frontLeftCalibrationValue);
    m_frontRightModule.set(states[1].speedMetersPerSecond * scale / MAX_VELOCITY_METERS_PER_SECOND * MAX_VOLTAGE,
        states[1].angle.getRadians() - _frontRightCalibrationValue);
    m_backLeftModule.set(states[2].speedMetersPerSecond * scale / MAX_VELOCITY_METERS_PER_SECOND * MAX_VOLTAGE,
        states[2].angle.getRadians() - _backLeftCalibrationValue);
    m_backRightModule.set(states[3].speedMetersPerSecond * scale / MAX_VELOCITY_METERS_PER_SECOND * MAX_VOLTAGE,
        states[3].angle.getRadians() - _backRightCalibrationValue);
  ...
  public SparkMaxPIDController getSteerPID() {
    return ((CANSparkMax) m_frontRightModule.getSteerMotor()).getPIDController();
  }
  ...
  public void setFrontRightSteerPID(double steerP, double steerI, double steerD) {
    CANSparkMax frontRight = (CANSparkMax) m_frontRightModule.getSteerMotor();
    frontRight.getPIDController().setP(steerP);
